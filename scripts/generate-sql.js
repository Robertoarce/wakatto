...
let sql = '-- Auto-generated system_prompt updates\n-- Run this in Supabase SQL Editor\n\n';

for (const [id, intro] of Object.entries(intros)) {
  const fullPrompt = intro + WAKATTOR_BASE;
  const escaped = escapeSQL(fullPrompt);
  sql += `UPDATE custom_wakattors SET system_prompt = '${escaped}' WHERE character_id = '${id}';\n\n`;
}

fs.writeFileSync('supabase/migrations/20251220000002_update_all_prompts.sql', sql);
console.log('✅ Generated SQL file with', Object.keys(intros).length, 'character updates');
console.log('📁 File: supabase/migrations/20251220000002_update_all_prompts.sql');
